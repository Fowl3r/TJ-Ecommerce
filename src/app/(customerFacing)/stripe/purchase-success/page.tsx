...
export default async function SuccessPage({searchParams} : {searchParams: {payment_intent:string}}){
    ...
    <h1 className="text-4xl font-bold">{isSucces ? "Payment Successful!" : "Error Payment Failed"}</h1>
    <div className="flex gap-4 items-center">
        ...
        <div>
            ...
            {!isSucces && (<Button className="mt-4" >{<Link href={`/products/${product.id}/purchase`}>Try again</Link> }</Button>)}
         </div>
    </div>
</div>
    )
